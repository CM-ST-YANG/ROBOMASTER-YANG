...
void UART3_Handler(void)
{
	...
	if (Read(&roll, 0) == 0xa5 && Read(&roll, 1) == 0x5a && Read(&roll, 12) == 0x5a && Read(&roll, 13) == 0xa5)
	{
		vision_receive_data[0] = Read(&roll, 3) + (Read(&roll, 2) << 8);
		vision_receive_data[1] = Read(&roll, 5) + (Read(&roll, 4) << 8);
		vision_receive_data[2] = Read(&roll, 7) + (Read(&roll, 6) << 8);
		vision_receive_data[3] = Read(&roll, 9) + (Read(&roll, 8) << 8);
		vision_receive_data[4] = Read(&roll, 11) + (Read(&roll, 10) << 8);
	}
